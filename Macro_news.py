...
folder_path = 'Macro_news'
output_file = 'news_analysis_results_宏观.csv'
...
def append_to_csv(news_data, filename):
    # Check if file exists and has content; if not, add header
    file_exists = os.path.isfile(filename) and os.path.getsize(filename) > 0
    with open(filename, mode='a', newline='', encoding='utf-8-sig') as file:
        fieldnames = ['发布时间', '新闻标题', 'URL', '影响评分']
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        if not file_exists:
            writer.writeheader()  # Only write header if file does not exist or is empty
    ...
    if filename.endswith(".xls") or filename.endswith(".xlsx"):
        ...
        for index, row in df.iterrows():
            ...
            if news_content:
                ...
                rating_response = analyze_news_with_openai(news_content)
